...
export class AdaptColorService {
  ...
  calculateLuminance({ r, g, b } : { r: number, g: number, b: number }){
  ...
  calculateContrastRatio(luminanceA: number, luminanceB: number){
  ...
  bestSuitedTextColor(color: string | undefined){
    return this.defineTextColorFromLuminance(
      this.calculateLuminance(
        this.convertHexToRgb(
          this.convertColorNameToHex(color)
        )
      )
    )
  }
